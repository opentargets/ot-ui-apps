...
function Stats({ classes }) {
  ...
  useEffect(() => {
    ...
    fetch("https://platform-api.opentargets.io/v3/platform/public/utils/stats")
      ...
      .then(data => {
        if (isCurrent) {
          const numDataSources = Object.values(data.associations.datatypes).reduce(
  ...
  return (
    <Grid className={classes.container} container justifyContent="center">
      <Grid item container direction="column" md={7} justifyContent="space-around">
        ...
        {stats && (
          <Grid container justifyContent="space-around">
            <Grid item container md={3} direction="column" alignItems="center">
              ...
              <Typography variant="h6">{stats.numTargets.toLocaleString()}</Typography>
              <Typography> targets</Typography>
            </Grid>
            <Grid item container md={3} direction="column" alignItems="center">
              <FontAwesomeIcon icon={faStethoscope} className={classes.icon} />
              <Typography variant="h6">{stats.numDiseases.toLocaleString()}</Typography>
              <Typography>diseases</Typography>
            </Grid>
            <Grid item container md={3} direction="column" alignItems="center">
              <Typography variant="h6">{stats.numAssociations.toLocaleString()}</Typography>
              <Typography>associations</Typography>
            </Grid>
            <Grid item container md={3} direction="column" alignItems="center">
              <Typography variant="h6">{stats.numDataSources.toLocaleString()}</Typography>
