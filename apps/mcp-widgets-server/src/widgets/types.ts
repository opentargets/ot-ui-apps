...
const CONFIG_PROFILE_SCRIPT =
  `window.configProfile = { isPartnerPreview: false, partnerTargetSectionIds: [], ` +
  `partnerDiseaseSectionIds: [], partnerDrugSectionIds: [], partnerEvidenceSectionIds: [], ` +
  `partnerDataTypes: [], partnerDataSources: [] };`;
...
<html>
  <head>
    ...
    <script>window.__OT_API_URL__ = "${apiUrl}"; ${CONFIG_PROFILE_SCRIPT}</script>
  </head>
  <body>
    <div id="root"></div>
  ...
  return {
    ...
    input_schema: {
      ...
      properties: Object.fromEntries(
        params.map(p => [p.name, { type: "string", description: p.description }])
      ),
