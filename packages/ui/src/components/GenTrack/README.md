...
| `onInnerScalesReady`   | `function`  |                | Callback fired once the inner track's `scalesRef` is ready. Receives the inner `RefObject<ScalesRef>`. Useful when content passed via `innerOverlayGraphics` needs access to the inner coordinate space (e.g. to position a `DataVLine` correctly).  |
