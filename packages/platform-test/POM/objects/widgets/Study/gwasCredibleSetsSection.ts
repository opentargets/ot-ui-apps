...
export class GWASCredibleSetsSection {
  ...
  // Get gene link
  getGeneLink(rowIndex: number): Locator {
    return this.getTableRows().nth(rowIndex).locator("a[href*='/target/']");
  }

  async clickGeneLink(rowIndex: number): Promise<void> {
    await this.getGeneLink(rowIndex).click();
  }

  async getGeneName(rowIndex: number): Promise<string | null> {
    return await this.getGeneLink(rowIndex).textContent();
  }
