    ...
    <thead>
      <tr>
        ...
        <Box
          component="th"
          colSpan="3"
          sx={{ borderBottom: "1px solid #888", paddingBottom: "3px" }}
        >
...
function getColorScale(groupResults) {
  ...
  // return scaleDiverging(t => interpolateRdBu(t * 0.6)).domain([min, max]);
  return scaleDiverging(interpolateRdBu).domain([min, 0, max]);
  // return scaleDiverging([min, 0, max], interpolateRdBu).range([0.2, 0.8]);
  // return scaleDiverging().domain([min, 0, max]).range([0.2, 0.8]);
}
